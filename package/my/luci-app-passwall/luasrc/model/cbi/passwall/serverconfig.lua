...
e=t:option(Value, "plugin", translate("plugin"))
e:value("none", "None")
if nixio.fs.access("/usr/bin/v2ray-plugin") then
e:value("/usr/bin/v2ray-plugin", "v2ray-plugin")
end
if nixio.fs.access("/usr/bin/obfs-local") then
e:value("/usr/bin/obfs-local", "obfs-local")
end
if nixio.fs.access("/usr/bin/gq-client") then
e:value("/usr/bin/gq-client", "GoQuiet")
end
e.rmempty = false
e.default = "none"
e:depends("server_type", "SS")

e=t:option(Value, "plugin_opts", translate("Plugin Opts"))
e.rmempty = true
e:depends("plugin", "/usr/bin/v2ray-plugin")
e:depends("plugin", "/usr/bin/obfs-local")
e:depends("plugin", "/usr/bin/gq-client")
